...
class TextManager {
	...
	func showMessage(_ text: String, autoHide: Bool = true) {
		DispatchQueue.main.async {
			...
			// set text
//			self.AR.messageLabel.text = text
	...
	func showAlert(title: String, message: String, actions: [UIAlertAction]? = nil) {
		...
		DispatchQueue.main.async {
			self.AR.present(self.alertController!, animated: true, completion: nil)
		}
	}
	...
	func blurBackground() {
		...
		blurEffectView.frame = AR.view.bounds
		blurEffectView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
		blurEffectView.tag = blurEffectViewTag
		AR.view.addSubview(blurEffectView)
	}
	
	func unblurBackground() {
		for view in AR.view.subviews {
	...
	private func showHideMessage(hide: Bool, animated: Bool) {
		...
		UIView.animate(withDuration: 0.2,
		               ...
		               animations: {
				//		self.AR.messageLabel.isHidden = hide
	...
	//	AR.messagePanel.isHidden = AR.messageLabel.isHidden
	}
